/**
 *  AWS Event Logger - SmartApp v 0.0.1
 *
 *  Author: 
 *    Alan Williams (alanjohnwilliams)
 *
 ...
 *
 *    0.0.1 (01/17/2018)
 *      -  Initial cut. Derived from IoT Logger (https://github.com/nicjansma/smart-things) and
 *         Simple Event Logger (https://github.com/krlaframboise/SmartThings/tree/master/smartapps/krlaframboise/simple-event-logger.src#simple-event-logger)
 * 
...
def mainPage() {
	dynamicPage(name:"mainPage", uninstall:true, install:true) {
		...
		section("  ") {
			...
			if (state.installed) {		
				getPageLink("aboutPageLink", "About AWS Event Logger", "aboutPage", null, "Tap to view documentation, version and additional information.", "https://raw.githubusercontent.com/krlaframboise/Resources/master/simple-event-logger/app-SimpleEventLogger@3x.png")
			}
		}
		section("  ") {
...
def aboutPage() {
	dynamicPage(name:"aboutPage") {
		section() {				
			paragraph image: "http://cdn.device-icons.smartthings.com/Home/home1-icn@3x.png",
				title: "AWS Event Logger\nBy Alan Williams (@AlanJohnWilliams)",
				required: false,
...
private getOptionsPageContent() {
	...
	section("${getWebAppName()}") {		
        input "httpUrl", "text", title: "URL", required: true
...
private verifyWebAppUrl(url,key) {
	if (!url) {
		logDebug "The Url field is required"
		return false
	}
	else if (!key) {
		logDebug "The x-api-key field is required"
		return false
	}
    return true
}
